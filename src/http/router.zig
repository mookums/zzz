...
pub fn Router(comptime Server: type) type {
    return struct {
        ...
        fn stat_file_task(rt: *Runtime, result: StatResult, provision: *FileProvision) !void {
            ...
            const stat = result.unwrap() catch {
                try provision.context.respond(.{
                    .status = .@"Not Found",
                    .mime = Mime.HTML,
                    .body = "File Not Found",
        ...
        fn read_file_task(rt: *Runtime, result: ReadResult, provision: *FileProvision) !void {
            ...
            const lengthi = result.unwrap() catch |e| {
                switch (e) {
                    error.EndOfFile => {
            ...
            const length: usize = @intCast(lengthi);
            provision.rd_offset += length;
            provision.current_length += length;
            log.debug("current offset: {d} | fd: {d}", .{ provision.rd_offset, provision.fd });

            assert(provision.rd_offset <= length);
