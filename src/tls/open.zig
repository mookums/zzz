...
const log = std.log.scoped(.@"zzz/tls/openssl");
...
pub const TLSContext = struct {
    ...
    pub fn init(options: TLSContextOptions) !TLSContext {
        ...
        _ = open.SSL_CTX_use_certificate_file(ctx, options.cert_path.ptr, open.SSL_FILETYPE_PEM);
        _ = open.SSL_CTX_use_PrivateKey_file(ctx, options.key_path.ptr, open.SSL_FILETYPE_PEM);
    ...
    pub fn create(self: TLSContext, socket: Socket) !TLS {
        ...
        return TLS.init(.{
            ...
            .tls = tls,
            .size_tls_buffer_max = 8192,
            .size_bio_buffer_max = 4096,
        });
    }
...
pub const TLS = struct {
    ...
    pub fn init(options: TLSOptions) !TLS {
        ...
        _ = open.BIO_set_write_buf_size(r_bio, options.size_bio_buffer_max);
        _ = open.BIO_set_write_buf_size(w_bio, options.size_bio_buffer_max);

        return TLS{
            .allocator = options.allocator,
            .buffer = try options.allocator.alloc(u8, options.size_tls_buffer_max),
            .tls = options.tls,
            .r_bio = r_bio,
            .w_bio = w_bio,
        };
